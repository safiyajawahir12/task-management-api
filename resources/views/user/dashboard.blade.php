...
<style>
  ...
  .modal { background:white; border-radius:12px; padding:2rem; width:100%; max-width:480px; box-shadow:0 20px 60px rgba(0,0,0,0.2); }
...
<div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:1.5rem;">
  <div>
    <h1 style="font-size:1.6rem; color:#1e293b;">👋 My Tasks</h1>
    <p style="color:#64748b; font-size:0.9rem;">Welcome, <strong>{{ Auth::user()->name }}</strong>. Tasks assigned to you.</p>
  </div>
</div>
...
<div class="card" style="padding:1rem; margin-bottom:1rem;">
  <form method="GET" action="{{ route('user.dashboard') }}" style="display:flex; gap:0.75rem; flex-wrap:wrap; align-items:flex-end;">
    <div>
    ...
    <a href="{{ route('user.dashboard') }}" class="btn" style="background:#6b7280;color:white;">Clear</a>
  </form>
</div>
      ...
      @forelse($tasks as $task)
        <tr>
          <td>
            ...
            @if($task->description)
              <p style="font-size:0.78rem;color:#6b7280;margin-top:0.2rem;">{{ Str::limit($task->description, 50) }}</p>
            @endif
          </td>
          ...
          <td>
            <div style="display:flex;gap:0.4rem;flex-wrap:wrap;">
              ...
              @if($task->status !== 'completed')
                <form method="POST" action="{{ route('user.tasks.complete', $task->id) }}">
                  @csrf
...
<script>
  function openEditModal(id, title, description, status, priority, dueDate) {
    document.getElementById('editTitle').value       = title;
    document.getElementById('editDescription').value = description;
    document.getElementById('editStatus').value      = status;
    document.getElementById('editPriority').value    = priority;
    document.getElementById('editDueDate').value     = dueDate;
    document.getElementById('editForm').action       = '/user/tasks/' + id;
